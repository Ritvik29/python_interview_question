...
- [Python Cơ Bản](#python-cơ-bản)
    - [Thao tác với tập tin](#thao-tác-với-tập-tin)
        - [1. Xử lý một tập tin file.txt có khoảng 10K dòng](#1-xử-lý-một-tập-tin-filetxt-có-khoảng-10k-dòng)
        - [2.补充缺失的代码](#2补充缺失的代码)
    - [模块与包](#模块与包)
        - [3.输入日期， 判断这一天是这一年的第几天？](#3输入日期-判断这一天是这一年的第几天)
        - [4. Sắp xếp ngẩu nhiên một danh sách alist](#4-sắp-xếp-ngẩu-nhiên-một-danh-sách-alist)
    - [Kiểu dữ liệu](#kiểu-dữ-liệu)
        - [5. Cho một từ điển (dict) d = {'a':24,'g':52,'i':12,'k':33} hãy sắp xếp theo khoá của từ điển](#5-cho-một-từ-điển-dict-d--a24g52i12k33-hãy-sắp-xếp-theo-khoá-của-từ-điển)
        - [6.字典推导式](#6字典推导式)
...
### 1. Xử lý một tập tin file.txt có khoảng 10K dòng
```python
def get_lines():
...
if __name__ == '__main__':
    for e in get_lines():
        process(e) # Xử lý từng dòng dữ liệu
```

Bây giờ tôi phải xử lý một tệp có kích thước 10G, nhưng bộ nhớ chỉ là 4G. Tôi nên làm gì nếu tôi chỉ sửa đổi hàm get_lines và mã khác vẫn giữ nguyên? Các vấn đề cần được xem xét là gì?
...
if __name__ == "__main__":
    file_path = 'file.txt'
    for i in get_lines(file_path):
        print(i)
```

Các vấn đề cần xem xét là: chỉ bộ nhớ 4G không thể đọc các tệp 10G tại một thời điểm và cần phải đọc theo lô và đọc dữ liệu theo lô để ghi lại vị trí của từng dữ liệu đã đọc. Kích thước của mỗi lô dữ liệu được đọc theo lô, quá nhỏ sẽ mất quá nhiều thời gian trong thao tác đọc
...
```
### 4. Sắp xếp ngẩu nhiên một danh sách alist？
```python
import random
alist = [1, 2, 3, 4, 5]
...
### 5. Cho một từ điển (dict) d = {'a':24,'g':52,'i':12,'k':33} hãy sắp xếp theo khoá của từ điển
```python
...
### 8. Chuyển chuỗi "k:1|k1:2|k2:3|k3:4" thành một dict {k:1, k1:2, ...}
```python
...
print(l2)
```
### 14. Cho hai danh sách (list) A，B hãy tìm các phần tử giống và khác nhau
```python
# Phần tử giống nhau A và B
print(set(A)&set(B))
# Phần tử khác nhau giữa A và B
print(set(A)^set(B))
```
